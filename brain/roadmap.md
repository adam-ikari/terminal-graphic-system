...
## Milestones
- **L0 — DONE (2026-09-15): a working character terminal.** The compositor *is* an xterm-class terminal: PTY (grid size set via forkpty's winsize before exec) → stream demultiplexer (text vs TGS APC frames) → VT emulator (`src/compositor/term.c`) → scene underlay (term_view pixels blitted under the widget scene). Keys return PTY bytes until a program speaks TGS. Verified: `htop` and `ls` run with zero TGS code; `tests/test_term.cpp` covers the emulator + demux; `term_probe` replays captured bytes.

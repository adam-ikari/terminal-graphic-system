 ...
 *
 * The renderer is a scene painter: it holds ONE canvas tree (the program's
 * whole drawing surface), runs hit-testing, and paints through a paint port.
 * Two paint ports exist: SDL2 (paint_sdl2.c) and Skia (paint_skia.c).
 * There is no window, focus, or navigation state — the program owns those.
 *
 * The paint port is the ONLY engine-specific seam — everything above it
 * (tree, state, events) is engine-independent C.
 */
#ifndef TGS_SCENE_H
#define TGS_SCENE_H
...
struct scene_node {
    ...
    /* Content (TEXT: the string; BOX: none). */
    char *text;                      /* malloc'd, NULL when none */

    /* Style overrides (set_element_style writes these; painting falls back
     * to per-kind defaults otherwise). */
...
typedef struct {
    scene_node *node;
    tgs_event_type type;
    char data[256];                  /* POINTER "x;y;phase" / KEY "code;mods" */
} scene_event;
...
typedef struct {
    ...
    scene_node *hovered;             /* last element the pointer entered */
...
scene_node *scene_create_node(tgs_scene *s, tgs_widget_type type,
                              scene_node *parent);   /* NULL parent = canvas root */
 ...
 * (w*h), blitted under the scene on every draw. NULL clears. */
...
/* Hit test: topmost visible element containing (ax, ay) in absolute
...
/* Drain events into the compositor callback (called from tick). */
void scene_pump_events(tgs_scene *s);

/* Per-kind visual defaults. */

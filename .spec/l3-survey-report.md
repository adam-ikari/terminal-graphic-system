...
widget types maps to a live 旧后端 object, 8 of 9 style props render, all four container types run the
native 旧后端 layout engines, per-window 旧后端 roots/groups + pointer activation work, and IME
...
three are in `scene_backend.c` / `window_manager.c` today.
...
| | Evidence |
|---|---|
| **Exists** | `TGS_STYLE_*` enum (9 props) `src/common/tgs_protocol.h:143-154`; `TGS_CMD_WGT_STYLE` (34) dispatched `window_manager.c:641-650` → `be->set_widget_style`; `backend_set_widget_style` `scene_backend.c:548-582`; client `tgs_client_set_widget_style` `tgs_client.c:325-340` |
| **Implemented** | `BG_COLOR`, `FG_COLOR`, `RADIUS`, `BORDER_WIDTH`, `BORDER_COLOR`, `SHADOW_WIDTH`, `SHADOW_COLOR`, `OPACITY` — all map 1:1 to 旧后端 style calls on the object's MAIN part (`scene_backend.c:553-577`) |
| **Stubbed** | `TGS_STYLE_FONT_SIZE` is a **silent no-op** (`scene_backend.c:578-580`, comment "旧后端 font size is fixed at compile time; skip"). Only `LV_FONT_MONTSERRAT_14` is compiled in (`后端配置:85-103`); the char base uses `lv_font_unscii_8` (`旧渲染引擎_term.c:14`). There is no runtime font switching, no way to name a font, and no scalable-font path |
| **Target (L3 "styles")** | All declared props visibly render; styles are queryable/reportable; font size actually changes text size |
| **Gap** | (1) `FONT_SIZE` no-op — needs ≥2 built-in fonts + a per-widget font selection path (ties into Resources, §E). (2) **No style query/report channel exists in the protocol at all** — styles are write-only fire-and-forget; an app cannot read back what it set, and there is no `GET_STYLE`/`NTF_STYLE`. (3) Styles apply only to the 旧后端 MAIN part; e.g. `FG_COLOR` set on a button does not restyle its child label (`set_widget_style` targets the object itself, not children). (4) No style *reset/theme* command. (5) No test asserts any style renders (`tests/` never mentions `TGS_STYLE_`) |
...
(`window_manager.c:446-476`) and get a live 旧后端 object in `create_旧渲染引擎_widget`
(`scene_backend.c:173-235`). Per-widget status:

| Widget | 旧后端 mapping | Functional? | Notes |
|---|---|---|---|
| BUTTON | `lv_button_create` (`:178`) | **Yes** | label child auto-created on content (`:476-485`); CLICK via `旧渲染引擎_event_handler` |
...
| RADIO | `lv_checkbox` + round radius (`:184-188`) | **Partial** | renders/toggles, but **no mutual-exclusion group** — 旧后端 has no radio group, and nothing else implements exclusivity |
...
| LIST | `lv_list_add_button` per content call (`:498-506`) | **Yes** | deprecated 旧后端 API, but works; appends rows |
...
| IMAGE | `lv_image_create` (`:209`) | **Stub** | created, but **nothing can load pixels**: all 旧后端 image decoders disabled (`后端配置:66-73`), no resource stream (§E), no content mapping (`:529-531`) — renders empty |
...
`test_旧渲染引擎_click.cpp` and `nav_probe.c` touch widgets; `nav_probe.c` is **not** in ctest).

**Effort:** **Small.** Per-stub work is localized and each has an 旧后端-native completion path.

## C. Containers

| | Evidence |
|---|---|
| **Exists** | Type-driven layout at creation: VLAYOUT/HLAYOUT set flex flow, GLAYOUT sets a grid descriptor array, SCROLL sets scrollable (`scene_backend.c:212-231`). New children of a grid parent are placed into the next free cell (`:448-450`, `grid_place_child` `:163-171`). Parent-of-container nesting works via `parent_id` (demo: `examples/container_demo.c:40-54`). After the 旧后端 tick the compositor reports real absolute geometry via `NTF_GEOMETRY` (69) for every container and its directly-laid-out children (`wm_flush_geometry` `window_manager.c:409-444`, gated on `geom_pending`; called from `main.c:437`); client caches it (`tgs_client.c:445-459`, `tgs_client_get_widget_geometry`) |
| **Runtime relayout** | `TGS_CMD_WGT_LAYOUT` (37) → `backend_set_widget_layout` (`scene_backend.c:584-603`): `FLEX_ROW`/`FLEX_COL` real, **`TGS_LAYOUT_GRID` is stubbed to `LV_FLEX_FLOW_ROW`** (`:596-598`) — a runtime switch to GRID silently becomes a row flex. Sets `geom_pending` so geometry is re-reported (`window_manager.c:662-663`) |
| **Target (L3 "containers")** | Containers actually arrange children (they already do via 旧后端's engines); GRID at runtime must be real; NTF_GEOMETRY reports each child's settled rect |
| **Gap** | (1) Runtime `TGS_LAYOUT_GRID` stub. (2) Grid template fixed 2-col/4-row — no protocol to size columns/rows/span. (3) No per-child flex grow/order, no gap/margin/align control (旧后端 supports these; nothing exposes them). (4) SCROLL: no way to set/view scroll offset, no scroll-value event. (5) Container test coverage is thin (`test_geometry.cpp` drives the *client* side of NTF_GEOMETRY over pipes; no test asserts 旧后端 actually stacks/grids children) |
| **Effort** | **Small.** The layout engines are 旧后端-native and already wired; the work is exposing grid config + fixing the runtime GRID stub |

## D. Multi-window

| | Evidence |
|---|---|
| **Exists** | Per-window 旧后端 root (100%×100% of the screen) + dedicated group (`backend_create_window` `scene_backend.c:379-419`); ring membership per window (`backend_set_window_ring` `:614-633`); keyboard hand-off to one active window (`backend_set_active_window` `:638-644`); compositor `activate_window` (`window_manager.c:128-154`) emits the lost/gained `NTF_FOCUS` pair and switches the 旧后端 keyboard. Pointer-click activation routes through `TGS_EVENT_FOCUS` → `activate_window` when the click lands on a widget of a background window (`:868-871`). WIN_CREATE activates the newest window (`:537`). Per-window focus restore: `nav_restore` (`:148`), successor via `nav_successor`/`nav_other_window` (`nav.h:129,138`). Focus reasons incl. `WINDOW_ACTIVATE`/`WINDOW_RESTORE`/`HIDDEN` (`tgs_protocol.h:79-80`) |
...
| `TGS_LAYOUT_GRID` (runtime, via 37) | **Stub → flex row** | `scene_backend.c:596-598` |
| `TGS_STYLE_FONT_SIZE` (8) | **No-op** | `scene_backend.c:578-580` |
...
1. App sets every `TGS_STYLE_*` prop on a button/label/input; each produces a visible 旧后端 change on screen (bg/fg/radius/border/shadow/opacity today; FONT_SIZE after a font mechanism lands).
2. `TGS_STYLE_FONT_SIZE` selects among ≥2 built-in font sizes (14px + one more), and text re-renders.
3. Acceptance test: a widget-render test (extends `test_旧渲染引擎_click.cpp` style harness or a new snapshot) sets a style and asserts the resulting surface differs.
...
1. §H.2 preedit overlay: `set_widget_preedit` added to `struct tgs_backend`, implemented in 旧后端 as an overlay, `IME_PREEDIT` un-stubbed; empty preedit clears; cancel (already sent) drops the overlay — and **the widget's own text is never polluted** by preedit.
...
- `src/backends/旧渲染引擎/scene_backend.c` — `create_旧渲染引擎_widget` (173-235), `set_widget_content` (469-533), `set_widget_style` (548-582), `set_widget_layout` (584-603), `create_window` (379-419)
